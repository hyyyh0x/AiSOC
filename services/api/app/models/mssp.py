...
class MSSPRulePackRule(Base):
    ...
    pack_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("mssp_rule_packs.id", ondelete="CASCADE"), primary_key=True
    )
    ...
    added_by_user: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("users.id", ondelete="SET NULL"), nullable=True
    )
...
class MSSPRulePackAssignment(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    pack_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("mssp_rule_packs.id", ondelete="CASCADE"), nullable=False, index=True
    )
    ...
    assigned_by_user: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("users.id", ondelete="SET NULL"), nullable=True
    )
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=lambda: datetime.now(UTC))
    updated_at: Mapped[datetime] = mapped_column(
    ...
    __table_args__ = (
        UniqueConstraint("pack_id", "child_tenant_id", name="mssp_rule_pack_assignments_pack_id_child_tenant_id_key"),
    )
...
class MSSPRuleOverride(Base):
    ...
    __table_args__ = (
        UniqueConstraint("child_tenant_id", "rule_id", name="mssp_rule_overrides_child_tenant_id_rule_id_key"),
    )
